...
function setPropVisible(fieldName,val){
...
function setPropReadonly(fieldName,val){
	var element = document.getElementsByName(fieldName);
...
function HandleFunctions(pstrTok, pStack, pdtFormat, parrVars, contextData) {
	...
	switch (varTmp) {
		...
		case "SETPROPERTYBYCONDITION":
			...
			objOp3 = arrArgs[1];
			objOp4 = arrArgs[0];
			var ctAmtWord = BCSUtill.Format.currentAmountWord(varTerm, objOp1,
		...
		case "GETSTOCKVALUE":
			...
			if (varTmp == "FORMATVALUE") {
				...
				var convert = BCSUtill.Format.formatValue(varTerm, objOp1,
						objOp2, objOp3);
				pStack.Push(convert);
		...
		case "FIRESQL" :
			...
			if (varTmp == "FORMATAMOUNT") {
				...
				var convertFormat = BCSUtill.Format.formatAmount(varTerm,
	 ...
	 */
	 ...
	 if (pj == 0) {
		...
		combObj = document.getElementById(getField);
			 ...
			 let des_json = storages['desc_json']['comp'];
		...
		if (result == "{}") {
			...
			if(storages['pagetype'] == 'txview'){	
				if (combObj.tagName == "ION-INPUT"){
					getFieldval = document.getElementsByName(getField)[1].value;
				}
					...
					if( storages['preset_value'][getField] != undefined){
						getFieldval = storages['preset_value'][getField];
					}
				}
			}
			else if(storages['pagetype'] == 'displayreport'){
...
function eventHandler(eventSubFn, eformId, contextData) {
	...
	validExpr = eventSubFn.substring(0, 2).match(findComment);
	if (!validExpr) {
		hasVariable = eventSubFn.split(":=");
		//ctEform = globalMap.get("EFORMID");
		ctEform = "Mobile Survey"

		if (hasVariable.length === 1) {
		
			return interfaceToExpr(hasVariable[0], contextData);

		} else {
			cache = hasVariable[0].replace(/^\s*/, "").replace(/\s*$/, "");
			resultSet = interfaceToExpr(hasVariable[1], contextData);
...
function interfaceToExpr(passFn, contextData) {
	...
	ctEform = ""
	ExprObj = new Expression("");
	ExprObj.Expression(passFn, ctEform, '');

	result = ExprObj.Evaluate(contextData);
...
function executeFireSQL(storeName, EformId, firesqlid, namedParam, sqltype, grid,selectedRecords) {
	...
	if (grid.toUpperCase() === 'R') {
		...
		response;
		if(selectedRecords){
			 param = constructFireSQLParamMultiRecord(storeName, firesqlid, namedParam, sqltype,selectedRecords);
		}else{
			param = constructFireSQLParam(storeName, firesqlid, namedParam, sqltype);
		}
		url = getURL() + "apps/fireSql/";
		param = param + "&username=" + globalMap.get("USERNAME") + "&projectid=" + globalMap.get("PROJECTNAME");

		bcs.app.ajaxhelper.doXhr(url, param, false, function (data) {

			if (data && data.toLowerCase() === "success") {
				console.log("FireSql successfully completed");
				return;
			} else if (data && data.toLowerCase() === "failure") {
				console.log("FireSql failed");
				return;
			}
		
			if (data) {
				response = Ext.JSON.decode(data);

				if (response) {
					eformVarMap.add("report-" + storeName.toLowerCase(), response);
				}
			}
		...
		if(ctEform=="Mast"){
			param = "firesqlid=" + firesqlid + "&storeName="
				+storeName + "&eformid=accounts_tree_txview&crud=" + sqltype + "&username=" + storages["USERNAME"] + "&projectid=" + storages["PROJECTID"] + "&ISMULTITENANT=" + storages["ISMULTITENANT"];

		}
		else{
			param = "firesqlid=" + firesqlid + "&storeName="
				+storeName + "&eformid=" + ctEform + "&crud=" + sqltype + "&username=" + storages["USERNAME"] + "&projectid=" + storages["PROJECTID"] + "&projectname=" +storages["PROJECTNAME"] + "&ISMULTITENANT=" + storages["ISMULTITENANT"] + "&type=" + grid;
		...
		if (trimNamedParam) {
			...
			}else if(grid.toUpperCase() === 'T' || grid.toUpperCase() === 'R') {
		...
		ajaxObj.onreadystatechange = function () {
			if (ajaxObj.readyState == 4 && ajaxObj.status == 200) {
				...
				try {
					...
					storages[ctEform + "-" + storeName.toLowerCase()] = jsonObj;
				} catch (err) {

					storages[ctEform + "-" + storeName.toLowerCase()] = "";
				}

				eformVarMap[ctEform + "-" + storeName.toLowerCase()] =  responseString;
				return true;
			}
		}
		ajaxObj.send(param);
...
function constructFireSQLParam(storeName, firesqlid, namedParam, sqltype) {
	...
	//paramObj['reportid'] = reportCtStatus.get("QID");
	...
	for (var k = 0; k < keyVal.length; k++) {
		...
		if (keyVal[k] !== null && keyVal[k].length > 0) {
			...
			if (val) {
				...
				paramObj[keyVal[k].toLowerCase()] = val;
			}
				// following elseid is handled in else itself
			/*else if(!val && reportEventVarMap.length>0){
				//if(!val){
				var mainPanel = Ext.getCmp('doc-body');
				var reportGrid = mainPanel.getActiveTab().items.items[0];
				if(reportGrid)
				{
					if(reportGrid.config.dataModel)
					{
						var getIviewVariable = reportGrid.config.dataModel.getIViewVariables();
						paramObj[keyVal[k].toLowerCase()] =getIviewVariable[keyVal[k].toLowerCase()];
					}
				}
				...
				if (reportGrid) {
					...
					if(reportGrid.config && reportGrid.config.dataModel)
					{
					
						var getIviewVariable = reportGrid.config.dataModel.getIViewVariables();
						if(getIviewVariable.hasOwnProperty(keyVal[k].toLowerCase())){
							paramObj[keyVal[k].toLowerCase()] =getIviewVariable[keyVal[k].toLowerCase()];
							continue;
						}
					}
					var currentRecord = null;
					if(reportGrid.model && reportGrid.model.get('current_record'))
					   currentRecord =reportGrid.model.get('current_record').data;
					if (currentRecord) {
						...
						if (columnsList) {
							...
							if (currentRecord && currentRecord[keyVal[k].toLowerCase()]) {

								if (column && column.dataType === "D") {
									var value =  currentRecord[keyVal[k].toLowerCase()];
									var dataValue = convertStringToDBFormat(value);
									paramObj[keyVal[k].toLowerCase()] = dataValue;
									continue;
								} else {

									paramObj[keyVal[k].toLowerCase()] =  currentRecord[keyVal[k].toLowerCase()];
									continue;
...
function processSqlGet(eformid,store, column) {
	...
	resultSet =  storages[ctEform + "-" + store.toLowerCase()];
	if(!resultSet){
     	resultSet = storages["report" + "-" + store.toLowerCase()];
    }
	if (resultSet) {
