...
class TxtbcomponentdetailsSerializer(serializers.ModelSerializer):
	enum_meta = serializers.SerializerMethodField()
	txtabledetailid = serializers.ReadOnlyField(source = "txtabledetailid.table_slug")
...
class TxtbdetailsSerializer(serializers.ModelSerializer):
	...
	def get_field_meta(self,obj):
		...
		product_main_category_serialized = TxtbcomponentdetailsSerializer(instance=txcomponent_meta,many=True)
		return product_main_category_serialized.data 
...
class TxtabledetailsSerializer(serializers.ModelSerializer):
	...
	def get_field_meta(self,obj):
		txcomponent_meta = Txtablecomponentdetails.objects.filter(Q(txtabledetailid_id=obj.id) & Q(is_system_component=False) & ~Q(status="deleted"))	
